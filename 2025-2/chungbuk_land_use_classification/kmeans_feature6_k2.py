...
# ===== 10) 클러스터 → 유형 라벨 자동 매핑 (k=2: 농업/산림형 vs 도시/산업형) =====
...
# urban_score가 낮은 쪽 → 농업/산림형, 높은 쪽 → 도시/산업형
order = cp["urban_score"].sort_values().index.tolist()  # [농업형쪽 군집, 도시형쪽 군집]

type_names = ["농업/산림형", "도시/산업형"]
...
# ===== 10-1) Softmax 기반 유형별 확률 계산 (k=2) =====
# KMeans는 X_scaled(4D 비율)에 대해 학습되었으므로
# kmeans.transform(X_scaled)로 각 클러스터까지 거리(N x 2)를 구하고
# softmax(-거리)로 2개 유형 확률을 계산
distances = kmeans.transform(X_scaled)  # shape (N, 2)
...
probs = np.apply_along_axis(softmax_neg_dist, 1, distances)  # (N, 2)

# cluster → 유형 역매핑으로 인덱스 구하기
...
print("\n=== 시군구별 대표 유형 (2015~2025 전체 기준 최빈값, k=2) ===")
...
print(f"\n=== {TARGET_YEAR}년 기준 시군구별 토지구성 + 인구 + 도로율 + 유형 (k=2) ===")
print(df_2025[[
    "토지소재명",
    "임야비율4", "농경지비율4", "대지비율4", "공장비율4",
    "도로율",
...
# ===== 14) PCA 평면 시각화 (2025년만, 유형별 색상, k=2) =====
plt.figure(figsize=(9, 7))

type_list = ["농업/산림형", "도시/산업형"]
color_map = {
    "농업/산림형": "#75CD97",   # 초록
    "도시/산업형": "#EA9358",  # 주황
}

# ===== 15) 2015~2025 전체 PCA + KMeans 결과 시각화 (텍스트는 2025년만, k=2) =====
...
plt.xlabel("PC1")
plt.ylabel("PC2")
plt.title(f"충청북도 토지이용 군집 (Kmenas k=2, 2015~2025년,\n임야/농경지/대지/공장 비율 + 도로율 + 인구밀도)")
plt.legend(title="유형")
plt.tight_layout()
