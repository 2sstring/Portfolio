...
feature_cols = [
    "임야비율4", "농경지비율4", "대지비율4", "공장비율4",
    "도로율", "pop_density"   # ← 6개 모두 KMeans에 사용
]
...
# ===== 9) KMeans (k=4) – 6차원(4비율+도로율+인구밀도)에 대해 수행 =====
...
# ===== 10) 클러스터 → 유형 라벨 자동 매핑 (도시형, 산업형, 산림형, 농업형) =====
cp = cluster_profile.copy()
available_idx = set(cp.index)

# 1) 산업형: 공장비율4가 가장 큰 클러스터
idx_ind = cp.loc[list(available_idx), "공장비율4"].idxmax()
cluster_ind = int(idx_ind)
available_idx.remove(idx_ind)

# 2) 도시형: 대지비율4가 가장 큰 클러스터
idx_city = cp.loc[list(available_idx), "대지비율4"].idxmax()
cluster_city = int(idx_city)
available_idx.remove(idx_city)

# 3) 농업형: 농경지비율4가 가장 큰 클러스터
idx_agri = cp.loc[list(available_idx), "농경지비율4"].idxmax()
cluster_agri = int(idx_agri)
available_idx.remove(idx_agri)

# 4) 산림형: 남은 1개 (임야비율4가 큰 쪽으로 남는 경우가 대부분)
idx_forest = list(available_idx)[0]
cluster_forest = int(idx_forest)
...
# ===== 10-1) Softmax 기반 유형별 확률 계산 (k=4) =====
distances = kmeans.transform(X_scaled)  # shape (N, 4)
...
probs = np.apply_along_axis(softmax_neg_dist, 1, distances)  # (N, 4)
...
# ===== 14) PCA 평면 시각화 (2015~2025 전체, 텍스트는 2025년만) =====
type_list = ["농업형", "산림형", "산업형", "도시형"]
color_map = {
    "농업형": "#E6C950",   # 노랑
    "산림형": "#75CD97",   # 초록
    "산업형": "#8FA1BB",   # 회색
...
plt.xlabel("PC1")
plt.ylabel("PC2")
plt.title("충청북도 토지이용 군집 (KMeans k=4, 2015~2025년,\n임야/농경지/대지/공장 비율 + 도로율 + 인구밀도)")
plt.legend(title="유형")
plt.tight_layout()
